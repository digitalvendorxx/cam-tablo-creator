...
let licenseModule;
try {
  licenseModule = require(path.join(ROOT, 'lib/license'));
} catch (err) {
  licenseModule = null;
}
const checks = [];
...
function check(name, fn) {
  try {
    const result = fn();
    if (result?.skip) {
      checks.push({ name, status: 'skip', detail: result.detail });
...
async function httpsGet(url, timeoutMs = 3000) {
  return new Promise((resolve, reject) => {
    const req = https.get(url, { timeout: timeoutMs }, (res) => {
      res.resume();
      res.on('end', () => resolve({ ok: true, detail: `HTTP ${res.statusCode}` }));
    });
    req.on('timeout', () => {
...
async function main() {
  console.log('=== Cam Tablo Creator - Saglik Kontrolu ===');
  console.log('');
  ...
  await checkAsync('Lisans (flowiqa.com)', async () => {
    if (!licenseModule) {
      return { skip: true, detail: 'Lisans modulu bundled kurulumda mevcut degil - UI uzerinden dogrulanir' };
    }

    const s = licenseModule.getStatus();
  ...
  await checkAsync('flowiqa lisans server erisim', async () => {
    return httpsGet('https://flowiqa.com', 3000);
  });
  ...
  for (const c of checks) {
    const icon = c.status === 'ok' ? '[OK]   ' : c.status === 'warn' ? '[WARN] ' : c.status === 'skip' ? '[SKIP] ' : '[FAIL] ';
