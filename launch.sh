...
REMOTE_VERSION=$(curl -sf --max-time 5 "https://www.flowiqa.com/api/version?app=etsy-cam-tablo" 2>/dev/null | node -e "try { let d=''; process.stdin.on('data', c=>d+=c); process.stdin.on('end', ()=>{ try { console.log(JSON.parse(d).version||'') } catch { console.log('') } }) }" 2>/dev/null || echo "")
...
if [ -n "$REMOTE_VERSION" ] && [ "$LOCAL_VERSION" != "$REMOTE_VERSION" ]; then
  ...
  if [ -z "$KEY" ]; then
    ...
    if curl -fsSL "https://www.flowiqa.com/install/etsy-cam-tablo.sh" | TARGET="$(pwd)" bash -s "$KEY" >/tmp/epc-update.log 2>&1; then
