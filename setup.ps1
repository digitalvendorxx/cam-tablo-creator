...
Write-Host "=== Cam Tablo Creator - Windows kurulum ===" -ForegroundColor Magenta
...
if (-not (Test-Path $envPath)) {
  ...
  $envContent = if (Test-Path $envExample) { [System.IO.File]::ReadAllText($envExample) } else { "" }
...
# 11. Masaustu kisayolu launch.bat'a point et (eski varsa overwrite)
$launchPath = Join-Path $Root "launch.bat"
if (-not (Test-Path $launchPath)) {
  Warn "launch.bat yok, kisayol atlandi"
} else {
  # Olasi tum desktop konumlari (OneDrive yonlendirmesi dahil).
  # Onemli: env var null ise Join-Path patlar -> once null kontrol et.
  $desktopRoots = @(
  ...
  ) | Where-Object { $_ -and ($_ -is [string]) -and ($_.Trim() -ne "") }

  $desktopCandidates = @()
  $desktopCandidates += [Environment]::GetFolderPath("Desktop")
  foreach ($root in $desktopRoots | Select-Object -Skip 1) {
    $desktopCandidates += Join-Path $root "Desktop"
  }
  $desktopCandidates = $desktopCandidates | Where-Object { $_ -and (Test-Path $_) } | Select-Object -Unique

  $created = $false
  foreach ($desk in $desktopCandidates) {
    $lnkPath = Join-Path $desk "Cam Tablo Creator.lnk"
    try {
      # Eski kisayolu sil (varsa) - eski yolu point ediyor olabilir
      if (Test-Path $lnkPath) {
        Remove-Item $lnkPath -Force -ErrorAction Stop
      }
      ...
      $lnk.Description = "flowiqa Cam Tablo Creator"
      $lnk.Save()
      Ok "Masaustu kisayolu: $lnkPath"
      $created = $true
    } catch {
      Warn "Kisayol olusturulamadi ($desk): $($_.Exception.Message)"
    }
  }

  if (-not $created) {
    Warn "Masaustu kisayolu HIC olusturulamadi. Manuel: '$launchPath' icin sag tik -> 'Send to Desktop'"
  }
}
...
Write-Host "  3. Tek tik baslatmak icin: masaustunden 'Cam Tablo Creator' kisayolu"
