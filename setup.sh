...
cat > "Cam Tablo Creator.command" <<'EOF'
#!/usr/bin/env bash
cd "$(dirname "$0")"
./launch.sh
EOF
chmod +x "Cam Tablo Creator.command"

if [ "$TARGET_DIR" != "$HOME/Desktop" ]; then
  ln -sf "$TARGET_DIR/Cam Tablo Creator.command" "$HOME/Desktop/Cam Tablo Creator.command"
fi
...
KALAN ADIMLAR:
  ...
  3. Tek tik baslatmak icin:
       Cam Tablo Creator.command dosyasina cift tikla
       Ilk seferde Gatekeeper icin sag tik > Open sec
       # update kontrol + browser + server + tarayici acilir
