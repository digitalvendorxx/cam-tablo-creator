# Cam Tablo Creator - Windows tek satir kurulum
# Kullanim:
#   iwr -useb https://raw.githubusercontent.com/esenbora/etsy-cam-tablo-app/release/install.ps1 | iex
# Ozel hedef:
#   $env:TARGET="C:\etsy-tool"; iwr ... | iex

$ErrorActionPreference = "Stop"

$REPO_URL = "https://github.com/esenbora/etsy-cam-tablo-app.git"
$BRANCH = if ($env:BRANCH) { $env:BRANCH } else { "release" }
$TARGET = if ($env:TARGET) { $env:TARGET } else { Join-Path $HOME "etsy-cam-tablo" }

Write-Host "=== Cam Tablo Creator - Windows tek satir kurulum ===" -ForegroundColor Cyan
